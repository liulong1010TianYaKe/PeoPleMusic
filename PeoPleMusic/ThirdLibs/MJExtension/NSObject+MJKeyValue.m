...
{
    if ([keyValues isKindOfClass:[self class]]) {
        return keyValues;
    }
...
- (void)setKeyValues:(NSDictionary *)keyValues
{
    ...
    [self enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
        ...
        if (!value || [value isKindOfClass:[NSNull class]]) {   //如果没有这个属性值，尝试把首字母转化为小写
            key = [KyoUtil getModelKey:key withUppercase:NO];
            value = keyValues[key];
            if (!value || [value isKindOfClass:[NSNull class]]) return; //如果转化为小写还是没有找到，则跳出
        }
